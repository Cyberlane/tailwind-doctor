...
func TestAdapterForKnowsVersion4(t *testing.T) {
	adapter, found := AdapterFor(Version4)
	if !found || adapter.Version() != Version4 {
		t.Fatalf("v4 adapter = %T, found %v", adapter, found)
	}
}
